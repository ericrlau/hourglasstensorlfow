	...
	def __call__(self, inputs):
		with tf.compat.v1.name_scope(self.name):
			with tf.compat.v1.name_scope('preprocessing'):
				pad_1 = tf.pad(tensor=inputs, paddings=np.array([[0,0],[2,2],[2,2],[0,0]]))
			...
			with tf.compat.v1.name_scope('stacks'):
				with tf.compat.v1.name_scope('hourglass.1'):
					...
					drop[0] = tf.compat.v1.layers.dropout(ll_[0], rate = 0.1, training = self.train)
				...
				for i in range(1, self.nbStack-1):
					with tf.compat.v1.name_scope('hourglass.' + str(i+1)):
						...
						drop[i] = tf.compat.v1.layers.dropout(ll_[i],rate=0.1, training = self.train)
				...
				with tf.compat.v1.name_scope('hourglass.' + str(self.nbStack)):
					...
					drop[self.nbStack-1] = tf.compat.v1.layers.dropout(ll[self.nbStack-1], rate=0.1, training = self.train)
