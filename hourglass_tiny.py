...
class HourglassModel():
    ...
    def generate_model(self):
        ...
        with tf.device(self.gpu):
            with tf.compat.v1.name_scope('inputs'):
                ...
                self.img = tf.compat.v1.placeholder(
                    dtype=tf.float32, shape=(None, 256, 256, 3), name='input_img')
                if self.w_loss:
                    self.weights = tf.compat.v1.placeholder(
                        dtype=tf.float32, shape=(None, self.outDim))
                # Shape Ground Truth Map: batchSize x nStack x 64 x 64 x outDim
                self.gtMaps = tf.compat.v1.placeholder(
                    dtype=tf.float32, shape=(None, self.nStack, 64, 64, self.outDim))
            ...
            with tf.compat.v1.name_scope('loss'):
                if self.w_loss:
                    self.loss = tf.reduce_mean(
                        input_tensor=self.weighted_bce_loss(), name='reduced_loss')
                else:
                    self.loss = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(
                        logits=self.output, labels=self.gtMaps), name='cross_entropy_loss')
            lossTime = time.time()
        ...
        with tf.device(self.cpu):
            ...
            with tf.compat.v1.name_scope('lr'):
        ...
        with tf.device(self.gpu):
            ...
            with tf.compat.v1.name_scope('minimizer'):
                self.update_ops = tf.compat.v1.get_collection(
                    tf.compat.v1.GraphKeys.UPDATE_OPS)
                with tf.control_dependencies(self.update_ops):
                    self.train_rmsprop = self.rmsprop.minimize(
                        self.loss, self.train_step)
            minimTime = time.time()
            print('---Minimizer : Done (' +
                  str(int(abs(optimTime-minimTime))) + ' sec.)')
        ...
        with tf.device(self.cpu):
            with tf.compat.v1.name_scope('training'):
                tf.compat.v1.summary.scalar(
                    'loss', self.loss, collections=['train'])
                tf.compat.v1.summary.scalar(
                    'learning_rate', self.lr, collections=['train'])
            with tf.compat.v1.name_scope('summary'):
                for i in range(len(self.joints)):
                    tf.compat.v1.summary.scalar(
                        self.joints[i], self.joint_accur[i], collections=['train', 'test'])
        self.train_op = tf.compat.v1.summary.merge_all('train')
        self.test_op = tf.compat.v1.summary.merge_all('test')
        self.weight_op = tf.compat.v1.summary.merge_all('weight')
        endTime = time.time()
        ...
        with tf.compat.v1.name_scope('Session'):
            with tf.device(self.gpu):
                ...
                if load is not None:
                    ...
                    print(
                        'Please give a Model in args (see README for further information)')

    def _train(self, nEpochs=10, epochSize=1000, saveStep=500, validIter=10):
        """
        """
        with tf.compat.v1.name_scope('Train'):
            self.generator = self.dataset._aux_generator(
                self.batchSize, self.nStack, normalize=True, sample_set='train')
            self.valid_gen = self.dataset._aux_generator(
                self.batchSize, self.nStack, normalize=True, sample_set='valid')
            startTime = time.time()
            self.resume = {}
            self.resume['accur'] = []
            self.resume['loss'] = []
            ...
            for epoch in range(nEpochs):
                ...
                for i in range(epochSize):
                    ...
                    tToEpoch = int((time.time() - epochstartTime)
                                   * (100 - percent)/(percent))
                    sys.stdout.write('\r Train: {0}>'.format("="*num) + "{0}>".format(" "*(20-num)) + '||' + str(percent)[
                                     :4] + '%' + ' -cost: ' + str(cost)[:6] + ' -avg_loss: ' + str(avg_cost)[:5] + ' -timeToEnd: ' + str(tToEpoch) + ' sec.')
                    sys.stdout.flush()
                    img_train, gt_train, weight_train = next(self.generator)
                    if i % saveStep == 0:
                        if self.w_loss:
                            _, c, summary = self.Session.run([self.train_rmsprop, self.loss, self.train_op], feed_dict={
                                                             self.img: img_train, self.gtMaps: gt_train, self.weights: weight_train})
                        else:
                            _, c, summary = self.Session.run([self.train_rmsprop, self.loss, self.train_op], feed_dict={
                                                             self.img: img_train, self.gtMaps: gt_train})
                        # Save summary (Loss + Accuracy)
                        self.train_summary.add_summary(
                            summary, epoch*epochSize + i)
                        self.train_summary.flush()
                    else:
                        if self.w_loss:
                            _, c, = self.Session.run([self.train_rmsprop, self.loss], feed_dict={
                                                     self.img: img_train, self.gtMaps: gt_train, self.weights: weight_train})
                        else:
                            _, c, = self.Session.run([self.train_rmsprop, self.loss], feed_dict={
                                                     self.img: img_train, self.gtMaps: gt_train})
                ...
                if self.w_loss:
                    ...
                    weight_summary = self.Session.run(
                        self.weight_op, {self.img: img_train, self.gtMaps: gt_train})
                ...
                print('Epoch ' + str(epoch) + '/' + str(nEpochs) + ' done in ' + str(int(epochfinishTime-epochstartTime)
                                                                                     ) + ' sec.' + ' -avg_time/batch: ' + str(((epochfinishTime-epochstartTime)/epochSize))[:4] + ' sec.')
                with tf.compat.v1.name_scope('save'):
                    self.saver.save(self.Session, os.path.join(
                        os.getcwd(), str(self.name + '_' + str(epoch + 1))))
                self.resume['loss'].append(cost)
                # Validation Set
                accuracy_array = np.array([0.0]*len(self.joint_accur))
                for i in range(validIter):
                    img_valid, gt_valid, w_valid = next(self.generator)
                    accuracy_pred = self.Session.run(self.joint_accur, feed_dict={
                                                     self.img: img_valid, self.gtMaps: gt_valid})
                    accuracy_array += np.array(accuracy_pred,
                                               dtype=np.float32) / validIter
                print('--Avg. Accuracy =', str((np.sum(accuracy_array) /
                      len(accuracy_array)) * 100)[:6], '%')
                self.resume['accur'].append(accuracy_pred)
                self.resume['err'].append(
                    np.sum(accuracy_array) / len(accuracy_array))
                valid_summary = self.Session.run(self.test_op, feed_dict={
                                                 self.img: img_valid, self.gtMaps: gt_valid})
            ...
            print('  Final Loss: ' + str(cost) + '\n' + '  Relative Loss: ' +
                  str(100*self.resume['loss'][-1]/(self.resume['loss'][0] + 0.1)) + '%')
            print('  Relative Improvement: ' +
                  str((self.resume['err'][-1] - self.resume['err'][0]) * 100) + '%')
            print('  Training Time: ' +
    ...
    def record_training(self, record):
        ...
        for line in range(len(record['accur'])):
            out_string = ''
            labels = [record['loss'][line]] + \
                [record['err'][line]] + record['accur'][line]
            for label in labels:
    ...
    def weighted_bce_loss(self):
        ...
        WORK IN PROGRESS
        """
        self.bceloss = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(
            logits=self.output, labels=self.gtMaps), name='cross_entropy_loss')
    ...
    def _accuracy_computation(self):
        ...
        for i in range(len(self.joints)):
            self.joint_accur.append(self._accur(
                self.output[:, self.nStack - 1, :, :, i], self.gtMaps[:, self.nStack - 1, :, :, i], self.batchSize))
    ...
    def _graph_hourglass(self, inputs):
        """Create the Network
        Args:
                # TODO : Create a parameter for customize size
                inputs : TF Tensor (placeholder) of shape (None, 256, 256, 3)
        """
        with tf.compat.v1.name_scope('model'):
            with tf.compat.v1.name_scope('preprocessing'):
                # Input Dim : nbImages x 256 x 256 x 3
                pad1 = tf.pad(tensor=inputs, paddings=[
                              [0, 0], [2, 2], [2, 2], [0, 0]], name='pad_1')
                # Dim pad1 : nbImages x 260 x 260 x 3
                conv1 = self._conv_bn_relu(
                    pad1, filters=64, kernel_size=6, strides=2, name='conv_256_to_128')
                ...
                if self.tiny:
                    ...
                    r2 = self._residual(
                        pool1, numOut=int(self.nFeat/2), name='r2')
            ...
            if self.tiny:
                with tf.compat.v1.name_scope('stacks'):
                    with tf.compat.v1.name_scope('stage_0'):
                        hg[0] = self._hourglass(
                            r3, self.nLow, self.nFeat, 'hourglass')
                        drop[0] = tf.compat.v1.layers.dropout(
                            hg[0], rate=self.dropout_rate, training=self.training, name='dropout')
                        ll[0] = self._conv_bn_relu(
                            drop[0], self.nFeat, 1, 1, name='ll')
                        if self.modif:
                            # TEST OF BATCH RELU
                            out[0] = self._conv_bn_relu(
                                ll[0], self.outDim, 1, 1, 'VALID', 'out')
                        else:
                            out[0] = self._conv(
                                ll[0], self.outDim, 1, 1, 'VALID', 'out')
                        out_[0] = self._conv(
                            out[0], self.nFeat, 1, 1, 'VALID', 'out_')
                        sum_[0] = tf.add_n([out_[0], ll[0], r3], name='merge')
                    for i in range(1, self.nStack - 1):
                        with tf.compat.v1.name_scope('stage_' + str(i)):
                            hg[i] = self._hourglass(
                                sum_[i-1], self.nLow, self.nFeat, 'hourglass')
                            drop[i] = tf.compat.v1.layers.dropout(
                                hg[i], rate=self.dropout_rate, training=self.training, name='dropout')
                            ll[i] = self._conv_bn_relu(
                                drop[i], self.nFeat, 1, 1, name='ll')
                            if self.modif:
                                # TEST OF BATCH RELU
                                out[i] = self._conv_bn_relu(
                                    ll[i], self.outDim, 1, 1, 'VALID', 'out')
                            else:
                                out[i] = self._conv(
                                    ll[i], self.outDim, 1, 1, 'VALID', 'out')
                            out_[i] = self._conv(
                                out[i], self.nFeat, 1, 1, 'VALID', 'out_')
                            sum_[i] = tf.add_n(
                                [out_[i], ll[i], sum_[i-1]], name='merge')
                    with tf.compat.v1.name_scope('stage_' + str(self.nStack - 1)):
                        hg[self.nStack - 1] = self._hourglass(
                            sum_[self.nStack - 2], self.nLow, self.nFeat, 'hourglass')
                        drop[self.nStack-1] = tf.compat.v1.layers.dropout(
                            hg[self.nStack-1], rate=self.dropout_rate, training=self.training, name='dropout')
                        ll[self.nStack - 1] = self._conv_bn_relu(
                            drop[self.nStack-1], self.nFeat, 1, 1, 'VALID', 'conv')
                        if self.modif:
                            out[self.nStack - 1] = self._conv_bn_relu(
                                ll[self.nStack - 1], self.outDim, 1, 1, 'VALID', 'out')
                        else:
                            out[self.nStack - 1] = self._conv(
                                ll[self.nStack - 1], self.outDim, 1, 1, 'VALID', 'out')
                if self.modif:
                ...
                with tf.compat.v1.name_scope('stacks'):
                    with tf.compat.v1.name_scope('stage_0'):
                        ...
                        drop[0] = tf.compat.v1.layers.dropout(
                            hg[0], rate=self.dropout_rate, training=self.training, name='dropout')
                        ll[0] = self._conv_bn_relu(
                            drop[0], self.nFeat, 1, 1, 'VALID', name='conv')
                        ll_[0] = self._conv(
                            ll[0], self.nFeat, 1, 1, 'VALID', 'll')
                        if self.modif:
                            # TEST OF BATCH RELU
                            out[0] = self._conv_bn_relu(
                                ll[0], self.outDim, 1, 1, 'VALID', 'out')
                    ...
                    for i in range(1, self.nStack - 1):
                        with tf.compat.v1.name_scope('stage_' + str(i)):
                            ...
                            drop[i] = tf.compat.v1.layers.dropout(
                                hg[i], rate=self.dropout_rate, training=self.training, name='dropout')
                            ll[i] = self._conv_bn_relu(
                                drop[i], self.nFeat, 1, 1, 'VALID', name='conv')
                            ll_[i] = self._conv(
                                ll[i], self.nFeat, 1, 1, 'VALID', 'll')
                            if self.modif:
                                out[i] = self._conv_bn_relu(
                                    ll[i], self.outDim, 1, 1, 'VALID', 'out')
                            else:
                            ...
                            out_[i] = self._conv(
                                out[i], self.nFeat, 1, 1, 'VALID', 'out_')
                            sum_[i] = tf.add_n(
                                [out_[i], sum_[i-1], ll_[0]], name='merge')
                    with tf.compat.v1.name_scope('stage_' + str(self.nStack - 1)):
                        hg[self.nStack - 1] = self._hourglass(
                            sum_[self.nStack - 2], self.nLow, self.nFeat, 'hourglass')
                        drop[self.nStack-1] = tf.compat.v1.layers.dropout(
    ...
    def _conv(self, inputs, filters, kernel_size=1, strides=1, pad='VALID', name='conv'):
        ...
        Args:
                ...
                strides		: Stride
                # DO NOT USE 'SAME' NETWORK BUILT FOR VALID
        ...
        with tf.compat.v1.name_scope(name):
            ...
            kernel = tf.Variable(tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=(
                "uniform" if False else "truncated_normal"))([kernel_size, kernel_size, inputs.get_shape().as_list()[3], filters]), name='weights')
            conv = tf.nn.conv2d(input=inputs, filters=kernel, strides=[
                                1, strides, strides, 1], padding=pad, data_format='NHWC')
            if self.w_summary:
                with tf.device('/cpu:0'):
                    tf.compat.v1.summary.histogram(
                        'weights_summary', kernel, collections=['weight'])
            return conv
    ...
    def _conv_bn_relu(self, inputs, filters, kernel_size=1, strides=1, pad='VALID', name='conv_bn_relu'):
        ...
        with tf.compat.v1.name_scope(name):
            kernel = tf.Variable(tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=(
                "uniform" if False else "truncated_normal"))([kernel_size, kernel_size, inputs.get_shape().as_list()[3], filters]), name='weights')
            conv = tf.nn.conv2d(input=inputs, filters=kernel, strides=[
                                1, strides, strides, 1], padding='VALID', data_format='NHWC')
            act_layer = tf.keras.layers.Activation(tf.nn.relu)
            norm = tf.keras.layers.BatchNormalization(
                momentum=0.9, epsilon=1e-5)
            normalized = norm(act_layer(conv), training=self.training)
            # norm = tf.keras.layers.BatchNormalization(
            #     conv, momentum=0.9, epsilon=1e-5, training=self.training, activation="relu")
        ...
        if self.tiny:
            with tf.compat.v1.name_scope(name):
                norm = tf.contrib.layers.batch_norm(
                    inputs, 0.9, epsilon=1e-5, activation_fn=tf.nn.relu, is_training=self.training)
                pad = tf.pad(tensor=norm, paddings=np.array(
                    [[0, 0], [1, 1], [1, 1], [0, 0]]), name='pad')
                conv = self._conv(pad, int(numOut), kernel_size=3,
                                  strides=1, pad='VALID', name='conv')
                return conv
        else:
            with tf.compat.v1.name_scope(name):
                with tf.compat.v1.name_scope('norm_1'):
                    act_layer = tf.keras.layers.Activation(tf.nn.relu)
                    norm_1 = tf.keras.layers.BatchNormalization(
                        momentum=0.9, epsilon=1e-5)
                    normalized_1 = norm_1(inputs, training=self.training)
                    normalized_1 = act_layer(normalized_1)
                    conv_1 = self._conv(normalized_1, int(
                        numOut/2), kernel_size=1, strides=1, pad='VALID', name='conv')
                with tf.compat.v1.name_scope('norm_2'):
                    act_layer = tf.keras.layers.Activation(tf.nn.relu)
                    norm_2 = tf.keras.layers.BatchNormalization(
                        momentum=0.9, epsilon=1e-5)
                    normalized_2 = norm_2(conv_1, training=self.training)
                    normalized_2 = act_layer(normalized_1)
                    pad = tf.pad(tensor=normalized_2, paddings=np.array(
                        [[0, 0], [1, 1], [1, 1], [0, 0]]), name='pad')
                    conv_2 = self._conv(
                        pad, int(numOut/2), kernel_size=3, strides=1, pad='VALID', name='conv')
                with tf.compat.v1.name_scope('norm_3'):
                    act_layer = tf.keras.layers.Activation(tf.nn.relu)
                    norm_3 = tf.keras.layers.BatchNormalization(
                        momentum=0.9, epsilon=1e-5)
                    normalized_3 = norm_3(conv_2, training=self.training)
                    normalized_3 = act_layer(normalized_3)
                    conv_3 = self._conv(normalized_3, int(
                        numOut), kernel_size=1, strides=1, pad='VALID', name='conv')
                return conv_3
    ...
    def _skip_layer(self, inputs, numOut, name='skip_layer'):
        ...
        with tf.compat.v1.name_scope(name):
            if inputs.get_shape().as_list()[3] == numOut:
                ...
                conv = self._conv(inputs, numOut, kernel_size=1,
                                  strides=1, name='conv')
                return conv
    ...
    def _hourglass(self, inputs, n, numOut, name='hourglass'):
        ...
        with tf.compat.v1.name_scope(name):
            ...
            # Lower Branch
            low_ = tf.keras.layers.MaxPool2D([2, 2], [2, 2], padding='VALID')
            low_ = low_(inputs)
            ...
            up_2 = tf.image.resize(low_3, tf.shape(input=low_3)[
                1:3]*2, name='upsampling', method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            if self.modif:
                # Use of RELU
    ...
    def _compute_err(self, u, v):
        ...
        return tf.divide(tf.sqrt(tf.square(tf.cast(u_x - v_x, dtype=tf.float32)) + tf.square(tf.cast(u_y - v_y, dtype=tf.float32))), tf.cast(91, dtype=tf.float32))
    ...
    def _accur(self, pred, gtMap, num_image):
        ...
        Returns:
                (float)
        """
        err = tf.cast(0, dtype=tf.float32)
        for i in range(num_image):
            err = tf.add(err, self._compute_err(pred[i], gtMap[i]))
        return tf.subtract(tf.cast(1, dtype=tf.float32), err/num_image)
    ...
    def _pool_layer(self, inputs, numOut, name='pool_layer'):
        with tf.compat.v1.name_scope(name):
            ...
            pool = tf.keras.layers.MaxPool2D([2, 2], [2, 2], padding='VALID')
            pool(bnr_1)
            pad_1 = tf.pad(tensor=pool, paddings=np.array(
                [[0, 0], [1, 1], [1, 1], [0, 0]]))
            conv_1 = self._conv(pad_1, numOut, kernel_size=3,
                                strides=1, name='conv')
            bnr_2 = self._bn_relu(conv_1)
            pad_2 = tf.pad(tensor=bnr_2, paddings=np.array(
                [[0, 0], [1, 1], [1, 1], [0, 0]]))
            conv_2 = self._conv(pad_2, numOut, kernel_size=3,
                                strides=1, name='conv')
            upsample = tf.image.resize(conv_2, tf.shape(input=conv_2)[
                1:3]*2, name='upsampling', method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        return upsample
    ...
    def _attention_iter(self, inputs, lrnSize, itersize, name='attention_iter'):
        with tf.compat.v1.name_scope(name):
            ...
            pad = tf.pad(tensor=inputs, paddings=np.array(
                [[0, 0], [1, 1], [1, 1], [0, 0]]))
            U = self._conv(pad, filters=1, kernel_size=3, strides=1)
            pad_2 = tf.pad(tensor=U, paddings=np.array(
                [[0, 0], [padding, padding], [padding, padding], [0, 0]]))
            sharedK = tf.Variable(tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=(
                "uniform" if False else "truncated_normal"))([lrnSize, lrnSize, 1, 1]), name='shared_weights')
            Q = []
            C = []
            for i in range(itersize):
                if i == 0:
                    conv = tf.nn.conv2d(input=pad_2, filters=sharedK, strides=[
                        1, 1, 1, 1], padding='VALID', data_format='NHWC')
                else:
                    conv = tf.nn.conv2d(
                        input=Q[i-1], filters=sharedK, strides=[1, 1, 1, 1], padding='SAME', data_format='NHWC')
                C.append(conv)
        ...
        with tf.compat.v1.name_scope(name):
            if usepart == 0:
                ...
                for i in range(partnum):
                    ...
                    pad = tf.pad(tensor=att, paddings=np.array(
                        [[0, 0], [0, 0], [0, 0], [0, 0]]))
    ...
    def _hg_mcam(self, inputs, n, numOut, imSize, nModual, name='mcam_hg'):
        with tf.compat.v1.name_scope(name):
            ...
            if n > 1:
                low2 = self._hg_mcam(
                    low[-1], n-1, numOut, int(imSize/2), nModual)
            else:
                low2 = self._residual(low[-1], numOut)
            low3 = self._residual(low2, numOut)
            up_2 = tf.image.resize(low3, tf.shape(input=low3)[
                                   1:3]*2, name='upsampling', method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    def _graph_mcam(self, inputs):
        ...
        with tf.compat.v1.name_scope('stacks'):
            for i in range(self.nStack):
                with tf.compat.v1.name_scope('houglass_' + str(i+1)):
                    hg = self._hg_mcam(
                        inter[i], self.nLow, self.nFeat, 64, nModual)

                if i == self.nStack - 1:
                    ll1 = self._lin(hg, self.nFeat*2)
                    ll2 = self._lin(ll1, self.nFeat*2)
                    drop = tf.compat.v1.layers.dropout(
                        ll2, rate=0.1, training=self.training)
                    ...
                    if i > self.nStack // 2:
                        ...
                        tmpOut = self._conv(
                            att, filters=self.outDim, kernel_size=1, strides=1)
                out.append(tmpOut)
                if i < self.nStack - 1:
                    outmap = self._conv(
                        tmpOut, filters=self.nFeat, kernel_size=1, strides=1)
