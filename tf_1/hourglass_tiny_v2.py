...
class HourglassModel():
	...
	def generate_model(self):
		...
		with tf.device(self.gpu):
			with tf.compat.v1.name_scope('inputs'):
				...
				self.img = tf.compat.v1.placeholder(dtype= tf.float32, shape= (None, 256, 256, 3), name = 'input_img')
				if self.w_loss:
					self.weights = tf.compat.v1.placeholder(dtype = tf.float32, shape = (None, self.outDim))
				# Shape Ground Truth Map: batchSize x nStack x 64 x 64 x outDim
				self.gtMaps = tf.compat.v1.placeholder(dtype = tf.float32, shape = (None, self.nStack, 64, 64, self.outDim))
			...
			with tf.compat.v1.name_scope('loss'):
				if self.w_loss:
					self.loss = tf.reduce_mean(input_tensor=self.weighted_bce_loss(), name='reduced_loss')
				else:
					self.loss = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=self.output, labels= self.gtMaps), name= 'cross_entropy_loss')
			lossTime = time.time()	
		...
		with tf.device(self.cpu):
			...
			with tf.compat.v1.name_scope('lr'):
				self.lr = tf.compat.v1.train.exponential_decay(self.learning_rate, self.train_step, self.decay_step, self.decay, staircase= True, name= 'learning_rate')
			lrTime = time.time()
			print('---LR : Done (' + str(int(abs(accurTime-lrTime))) + ' sec.)')
		with tf.device(self.gpu):
			with tf.compat.v1.name_scope('rmsprop'):
				self.rmsprop = tf.compat.v1.train.RMSPropOptimizer(learning_rate= self.lr)
			optimTime = time.time()
		...
		with tf.device(self.cpu):
			with tf.compat.v1.name_scope('training'):
				tf.compat.v1.summary.scalar('loss', self.loss, collections = ['train'])
				tf.compat.v1.summary.scalar('learning_rate', self.lr, collections = ['train'])
			with tf.compat.v1.name_scope('summary'):
				for i in range(len(self.joints)):
					tf.compat.v1.summary.scalar(self.joints[i], self.joint_accur[i], collections = ['train', 'test'])
	...
	def weighted_bce_loss(self):
		...
		WORK IN PROGRESS
		"""
		self.bceloss = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=self.output, labels= self.gtMaps), name= 'cross_entropy_loss')
	...
	def _define_saver_summary(self, summary = True):
		...
		if (self.logdir_train == None) or (self.logdir_test == None):
			...
			if summary:
				with tf.device(self.gpu):
					self.train_summary = tf.compat.v1.summary.FileWriter(self.logdir_train, tf.compat.v1.get_default_graph())
					self.test_summary = tf.compat.v1.summary.FileWriter(self.logdir_test)
	...
	def _graph_hourglass(self, inputs):
		...
		with tf.compat.v1.name_scope('model'):
			with tf.compat.v1.name_scope('preprocessing'):
				...
				pad1 = tf.pad(tensor=inputs, paddings=[[0,0],[2,2],[2,2],[0,0]], name='pad_1')
			...
			if self.tiny:
				with tf.compat.v1.name_scope('stacks'):
					with tf.compat.v1.name_scope('stage_0'):
						...
						drop[0] = tf.compat.v1.layers.dropout(hg[0], rate = self.dropout_rate, training = self.training, name = 'dropout')
					...
					for i in range(1, self.nStack - 1):
						with tf.compat.v1.name_scope('stage_' + str(i)):
							...
							drop[i] = tf.compat.v1.layers.dropout(hg[i], rate = self.dropout_rate, training = self.training, name = 'dropout')
					...
					with tf.compat.v1.name_scope('stage_' + str(self.nStack - 1)):
						...
						drop[self.nStack-1] = tf.compat.v1.layers.dropout(hg[self.nStack-1], rate = self.dropout_rate, training = self.training, name = 'dropout')
		...
		with tf.compat.v1.name_scope(name):
			...
			if self.w_summary:
				with tf.device('/cpu:0'):
					tf.compat.v1.summary.histogram('weights_summary', kernel, collections = ['weight'])
			return conv
	...
	def _conv_block(self, inputs, numOut, name = 'conv_block'):
		...
		if self.tiny:
			with tf.compat.v1.name_scope(name):
				...
				pad = tf.pad(tensor=norm, paddings=np.array([[0,0],[1,1],[1,1],[0,0]]), name= 'pad')
			...
			with tf.compat.v1.name_scope(name):
				...
				with tf.compat.v1.name_scope('norm_2'):
					...
					pad = tf.pad(tensor=norm_2, paddings=np.array([[0,0],[1,1],[1,1],[0,0]]), name= 'pad')
	...
	def _hourglass(self, inputs, n, numOut, name = 'hourglass'):
		...
		with tf.compat.v1.name_scope(name):
			...
			up_2 = tf.image.resize(low_3, tf.shape(input=low_3)[1:3]*2, name = 'upsampling', method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
			if self.modif:
				# Use of RELU
	...
	def _pool_layer(self, inputs, numOut, name = 'pool_layer'):
		with tf.compat.v1.name_scope(name):
			...
			pad_1 = tf.pad(tensor=pool, paddings=np.array([[0,0],[1,1],[1,1],[0,0]]))
			conv_1 = self._conv(pad_1, numOut, kernel_size=3, strides=1, name='conv')
			bnr_2 = self._bn_relu(conv_1)
			pad_2 = tf.pad(tensor=bnr_2, paddings=np.array([[0,0],[1,1],[1,1],[0,0]]))
			conv_2 = self._conv(pad_2, numOut, kernel_size=3, strides=1, name='conv')
			upsample = tf.image.resize(conv_2, tf.shape(input=conv_2)[1:3]*2, name = 'upsampling', method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
		return upsample
	...
	def _attention_iter(self, inputs, lrnSize, itersize, name = 'attention_iter'):
		with tf.compat.v1.name_scope(name):
			...
			pad = tf.pad(tensor=inputs, paddings=np.array([[0,0],[1,1],[1,1],[0,0]]))
			U = self._conv(pad, filters=1, kernel_size=3, strides=1)
			pad_2 = tf.pad(tensor=U, paddings=np.array([[0,0],[padding,padding],[padding,padding],[0,0]]))
			sharedK = tf.Variable(tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution=("uniform" if False else "truncated_normal"))([lrnSize,lrnSize, 1, 1]), name= 'shared_weights')
			Q = []
			C = []
			for i in range(itersize):
				if i ==0:
					conv = tf.nn.conv2d(input=pad_2, filters=sharedK, strides=[1,1,1,1], padding='VALID', data_format='NHWC')
				else:
					conv = tf.nn.conv2d(input=Q[i-1], filters=sharedK, strides=[1,1,1,1], padding='SAME', data_format='NHWC')
				C.append(conv)
		...
		with tf.compat.v1.name_scope(name):
			if usepart == 0:
				...
				for i in range(partnum):
					...
					pad = tf.pad(tensor=att, paddings=np.array([[0,0],[0,0],[0,0],[0,0]]))
	...
	def _hg_mcam(self, inputs, n, numOut, imSize, nModual, name = 'mcam_hg'):
		with tf.compat.v1.name_scope(name):
			...
			up_2 = tf.image.resize(low3, tf.shape(input=low3)[1:3]*2, name = 'upsampling', method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
	...
	def _graph_mcam(self, inputs):
		with tf.compat.v1.name_scope('preprocessing'):
			pad1 = tf.pad(tensor=inputs, paddings=np.array([[0,0],[3,3],[3,3],[0,0]]))
		...
		with tf.compat.v1.name_scope('stacks'):
			for i in range(self.nStack):
				...
				if i == self.nStack - 1:
					...
					drop = tf.compat.v1.layers.dropout(ll2, rate=0.1, training = self.training)
