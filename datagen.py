...
class DataGenerator():
    ...
    def __init__(self, joints_name=None, img_dir=None, train_data_file=None, remove_joints=None):
        ...
        """
        if joints_name == None:
            self.joints_list = ['r_anckle', 'r_knee', 'r_hip', 'l_hip', 'l_knee', 'l_anckle', 'pelvis',
                                'thorax', 'neck', 'head', 'r_wrist', 'r_elbow', 'r_shoulder', 'l_shoulder', 'l_elbow', 'l_wrist']
        else:
            ...
            if joints == [-1] * len(joints):
                ...
                for i in range(joints.shape[0]):
                    if np.array_equal(joints[i], [-1, -1]):
                        w[i] = 0
                self.data_dict[name] = {'box': box,
                                        'joints': joints, 'weights': w}
    ...
    def _generate_hm(self, height, width, joints, maxlenght, weight):
        ...
        for i in range(num_joints):
            if not(np.array_equal(joints[i], [-1, -1])) and weight[i] == 1:
                ...
                hm[:, :, i] = self._makeGaussian(
                    height, width, sigma=s, center=(joints[i, 0], joints[i, 1]))
            else:
    ...
    def _crop_data(self, height, width, box, joints, boxp=0.05):
        ...
        if crop_box[1] < 0:
            crop_box[1] = 0
        if crop_box[2] > width - 1:
            crop_box[2] = width - 1
        if crop_box[3] > height - 1:
            crop_box[3] = height - 1
        new_h = int(crop_box[3] - crop_box[1])
        new_w = int(crop_box[2] - crop_box[0])
        crop_box = [crop_box[0] + new_w // 2,
                    crop_box[1] + new_h // 2, new_w, new_h]
        if new_h > new_w:
    ...
    def _crop_img(self, img, padding, crop_box):
        ...
        img = img[crop_box[1] - max_lenght // 2:crop_box[1] + max_lenght //
                  2, crop_box[0] - max_lenght // 2:crop_box[0] + max_lenght // 2]
        return img
    ...
    def _crop(self, img, hm, padding, crop_box):
        ...
        img = img[crop_box[1] - max_lenght // 2:crop_box[1] + max_lenght //
                  2, crop_box[0] - max_lenght // 2:crop_box[0] + max_lenght // 2]
        hm = hm[crop_box[1] - max_lenght // 2:crop_box[1] + max_lenght //
                2, crop_box[0] - max_lenght // 2:crop_box[0] + max_lenght // 2]
        return img, hm
    ...
    def _generator(self, batch_size=16, stacks=4, set='train', stored=False, normalize=True, debug=False):
        ...
        while True:
            ...
            train_gtmap = np.zeros(
                (batch_size, stacks, 64, 64, len(self.joints_list)), np.float32)
            ...
            for i, name in enumerate(files):
                if name[:-1] in self.images:
                    try:
                        ...
                        if debug:
                            print(box)
                        padd, cbox = self._crop_data(
                            img.shape[0], img.shape[1], box, joints, boxp=0.2)
                        if debug:
                            print(cbox)
                            print('maxl :', max(cbox[2], cbox[3]))
                        new_j = self._relative_joints(
                            cbox, padd, joints, to_size=64)
    ...
    def _aux_generator(self, batch_size=16, stacks=4, normalize=True, sample_set='train'):
        ...
        while True:
            ...
            train_weights = np.zeros(
                (batch_size, len(self.joints_list)), np.float32)
            i = 0
            while i < batch_size:
                try:
                    if sample_set == 'train':
    ...
    def open_img(self, name, color='RGB'):
        ...
        if color == 'RGB':
            ...
            print(
                'Color mode supported: RGB/BGR. If you need another mode do it yourself :p')
    ...
    def test(self, toWait=0.2):
        ...
        for i in range(len(self.train_set)):
            ...
            padd, box = self._crop_data(img.shape[0], img.shape[1], self.data_dict[self.train_set[i]]
                                        ['box'], self.data_dict[self.train_set[i]]['joints'], boxp=0.0)
            new_j = self._relative_joints(
                box, padd, self.data_dict[self.train_set[i]]['joints'], to_size=256)
        ...
        for s in self.data_dict.keys():
            if testSet == None:
                if self.data_dict[s]['weights'][id_lhip] == 1 and self.data_dict[s]['weights'][id_rsho] == 1:
                    ...
                    wIntel = np.unique(
                        self.data_dict[s]['weights'], return_counts=True)
