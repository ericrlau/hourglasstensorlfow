...
class YOLONet(object):
    ...
    def __init__(self, is_training=True):
        ...
        if is_training:
            self.labels = tf.compat.v1.placeholder(tf.float32, [None, self.cell_size, self.cell_size, 5 + self.num_class])
    ...
    def build_network(self,
        ...
        with tf.compat.v1.variable_scope(scope):
            with slim.arg_scope([slim.conv2d, slim.fully_connected],
                ...
                net = tf.pad(tensor=images, paddings=np.array([[0, 0], [3, 3], [3, 3], [0, 0]]), name='pad_1')
                ...
                net = tf.pad(tensor=net, paddings=np.array([[0, 0], [1, 1], [1, 1], [0, 0]]), name='pad_27')
                net = slim.conv2d(net, 1024, 3, 2, padding='VALID', scope='conv_28')
                net = slim.conv2d(net, 1024, 3, scope='conv_29')
                net = slim.conv2d(net, 1024, 3, scope='conv_30')
                net = tf.transpose(a=net, perm=[0, 3, 1, 2], name='trans_31')
    ...
    def calc_iou(self, boxes1, boxes2, scope='iou'):
        ...
        with tf.compat.v1.variable_scope(scope):
            boxes1 = tf.stack([boxes1[:, :, :, :, 0] - boxes1[:, :, :, :, 2] / 2.0,
                               ...
                               boxes1[:, :, :, :, 1] + boxes1[:, :, :, :, 3] / 2.0])
            boxes1 = tf.transpose(a=boxes1, perm=[1, 2, 3, 4, 0])

            boxes2 = tf.stack([boxes2[:, :, :, :, 0] - boxes2[:, :, :, :, 2] / 2.0,
                               boxes2[:, :, :, :, 1] - boxes2[:, :, :, :, 3] / 2.0,
                               boxes2[:, :, :, :, 0] + boxes2[:, :, :, :, 2] / 2.0,
                               boxes2[:, :, :, :, 1] + boxes2[:, :, :, :, 3] / 2.0])
            boxes2 = tf.transpose(a=boxes2, perm=[1, 2, 3, 4, 0])
    ...
    def loss_layer(self, predicts, labels, scope='loss_layer'):
        with tf.compat.v1.variable_scope(scope):
            ...
            predict_boxes_tran = tf.stack([(predict_boxes[:, :, :, :, 0] + offset) / self.cell_size,
                                           (predict_boxes[:, :, :, :, 1] + tf.transpose(a=offset, perm=(0, 2, 1, 3))) / self.cell_size,
                                           tf.square(predict_boxes[:, :, :, :, 2]),
                                           tf.square(predict_boxes[:, :, :, :, 3])])
            predict_boxes_tran = tf.transpose(a=predict_boxes_tran, perm=[1, 2, 3, 4, 0])

            iou_predict_truth = self.calc_iou(predict_boxes_tran, boxes)

            # calculate I tensor [BATCH_SIZE, CELL_SIZE, CELL_SIZE, BOXES_PER_CELL]
            object_mask = tf.reduce_max(input_tensor=iou_predict_truth, axis=3, keepdims=True)
            ...
            boxes_tran = tf.stack([boxes[:, :, :, :, 0] * self.cell_size - offset,
                                   boxes[:, :, :, :, 1] * self.cell_size - tf.transpose(a=offset, perm=(0, 2, 1, 3)),
                                   tf.sqrt(boxes[:, :, :, :, 2]),
                                   tf.sqrt(boxes[:, :, :, :, 3])])
            boxes_tran = tf.transpose(a=boxes_tran, perm=[1, 2, 3, 4, 0])

            # class_loss
            class_delta = response * (predict_classes - classes)
            class_loss = tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=tf.square(class_delta), axis=[1, 2, 3]), name='class_loss') * self.class_scale

            # object_loss
            object_delta = object_mask * (predict_scales - iou_predict_truth)
            object_loss = tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=tf.square(object_delta), axis=[1, 2, 3]), name='object_loss') * self.object_scale

            # noobject_loss
            noobject_delta = noobject_mask * predict_scales
            noobject_loss = tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=tf.square(noobject_delta), axis=[1, 2, 3]), name='noobject_loss') * self.noobject_scale

            # coord_loss
            coord_mask = tf.expand_dims(object_mask, 4)
            boxes_delta = coord_mask * (predict_boxes - boxes_tran)
            coord_loss = tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=tf.square(boxes_delta), axis=[1, 2, 3, 4]), name='coord_loss') * self.coord_scale
